...
void proximity(std::vector<std::vector<float>> points,int i,std::vector<int>& cluster,std::vector<int>& mark,KdTree* tree, float distanceTol)
{
   ...
   std::vector<int> nearbypoints = tree->search(points[i],distanceTol);
  	for(std::vector<int>::iterator it=nearbypoints.begin(); it != nearbypoints.end(); ++it)
	  {
		  ...
		  {
			  proximity(points,*it,cluster,mark,tree,distanceTol);
		  }
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for(int i=0; i< points.size(); i++)
	{
        if(mark[i] == 0) //if point has not been processed
		{
			...
			proximity(points,i,cluster,mark,tree,distanceTol);
